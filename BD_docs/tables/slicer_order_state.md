...
| `claimed_by_uuid` | VARCHAR(255) | ❌ | `NULL` | 029 | `users.uuid` нарезчика, который взял позицию в работу. NULL = свободна |
| `claimed_by_name` | VARCHAR(255) | ❌ | `NULL` | 029 | `users.login` на момент клейма (денормализовано: чужую `users` не джойним на каждый поллинг) |
| `claimed_at` | TIMESTAMPTZ | ❌ | `NULL` | 029 | Когда взяли в работу. Тайбрейкер владельца склеенной карточки |
...
Метка «эту карточку уже режут» хранится здесь и раздаётся всем планшетам обычным `GET /api/orders` (поллинг 4 сек). До миграции 029 она жила в локальном стейте одного браузера и второй нарезчик про неё не знал.

| Событие | claimed_by_uuid / name / at |
|---|---|
| `POST /api/orders/claim` (тап по свободной карточке) | ← актор из PIN-сессии, `claimed_at = NOW()` |
| `POST /api/orders/claim` на позицию, занятую ДРУГИМ | вся транзакция откатывается, ответ `409` + имя владельца |
| `POST /api/orders/unclaim` (повторный тап по своей) | → NULL. Чужой клейм не снимается: `WHERE claimed_by_uuid = актор` |
| `/park` (ручная парковка) | → NULL (заказ отложен, нарезчик переключился) |
| Разморозка (`defrost-*`) | не трогаем — рыбу доделает тот же человек |
| `/restore` | → NULL (позиция вернулась из истории «чистым листом») |
| «Готово» / «Частично» | строка уходит в `COMPLETED` вместе с клеймом; в историю пишется тот, кто НАЖАЛ (см. `slicer_order_history.completed_by_*`) |

Карточка в агрегированных режимах склеена из нескольких позиций. Она считается занятой, если занята **хотя бы одна** позиция; владелец — тот, чей `claimed_at` самый ранний (одинаковый ответ на всех планшетах, как бы очередь ни разбила группу).
