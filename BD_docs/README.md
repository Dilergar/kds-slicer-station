...
- **Database:** arclient
- **User:** postgres
- **Password:** задаётся при развёртывании в `server/.env` (`DB_PASSWORD`), см. `server/.env.example`

> Пароль намеренно не указан в документации. Раньше здесь стояло значение
> со стенда разработки, и этот же документ входит в комплект, по которому
> IT-команда заказчика разворачивает модуль — то есть предлагал им слабый
> пароль как рабочий. Своё значение задайте в `server/.env`.
