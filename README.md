...
│   └── migrations/            # SQL миграции (30 файлов: 001–030)
...
│   ├── migrations/            # Описание миграций (30 файлов)
...
| `slicer_order_state` | Состояние заказов нарезчика (парковка, статус, finished_at, defrost, клейм «В работе») |
| `slicer_order_history` | Завершённые заказы (KPI) + автор порции |
...
### Режим 2–3 нарезчиков (миграция 029)
Тап по карточке помечает её «В работе», и метку видят **все планшеты**: своя карточка — лаймовая рамка, чужая — голубая, у обеих подпись «🔪 имя нарезчика». Два человека могут резать в разных концах кухни и не браться за одно блюдо дважды.
- Повторный тап отпускает карточку. Чужую отпустить нельзя — клейм висит до «Готово» (парковка тоже снимает, разморозка сохраняет).
- «Готово»/«Частично» на чужой карточке спрашивают подтверждение: «Карточку взял Азамат. Всё равно отдать?»
- Имя берётся из PIN-сессии, поэтому **у нарезчиков должны быть разные PIN** в таблице `users`.

### Столы-домики (миграция 030)
Столы, вынесенные из зала в отдельные домики (по умолчанию 30, 31, 32), помечаются на карточке: их номер рисуется **кирпичным цветом в рамке в форме домика** вместо обычного жёлтого. Нарезчик видит «это на улицу» прямо по номеру стола.
- Список номеров правится в UI: **Админка → Общие Настройки → «Столы-домики»** (добавить номер / убрать крестиком), без правок кода.
- Метится **только номер стола** — оформление самой карточки не меняется.
- Фиолетовая рамка вокруг номера означает другое («стол парковался в смене») и сохраняется: у стола-домика, который парковался, домик кирпичный, а заливка и цифра внутри — фиолетовые.
...
### Dashboard KPI
- Средняя скорость приготовления (обычные vs паркованные) — с автором каждой порции и фильтром по нарезчикам (029)
...
# ... и далее строго по номерам до 030_house_tables.sql
# (все 30 команд по порядку — в Инструкция.md, раздел 4.2)
```
