...
> Актуализирован 2026-08-15 (передача): **аудит документации перед передачей IT-команде заказчика.** Сверены с фактическим кодом `README.md`, `Инструкция.md`, `Полная_инструкция_по_установке.txt`, `BD_docs/`. Главная находка — не документационная: `npm run migrate` в `server/package.json` обрывался на 029, миграции 030 в списке не было, хотя все три документа обещали «001–030». У заказчика это дало бы работающий модуль, в котором молча не сохраняется НИ ОДНА настройка (`PUT /api/settings` пишет `house_tables`). Скрипт дополнен. Остальное — расхождения текста с фактом: права БД без `users`/`userroles`/`roles` (вход по PIN не работал бы), `.env` без `DB_TIMEZONE`/`SERVER_HOST`/`CORS_ORIGINS`, строки лога старта на английском, которых код не печатает, пошаговая инструкция с миграциями до 026, отсутствующие в README эндпоинты клейма и `/api/auth/me`. Детали каждой правки — ниже по тексту и в самих файлах. Тогда же **пересобран `slicer_data_dump.sql`** (215 КБ, 9 таблиц: 472 рецепта, 260 ингредиентов, 116 алиасов): килсвитч синхронизации приведён в `f`, из шапки убраны конструкции PG 17+, восстановление проверено прогоном в чистую БД. Пункты 6–7 «Известных проблем» этим закрыты; процедура пересборки — в `Инструкция.md` шаг 4.6.
>
> Предыдущая веха — 2026-08-15 (вечер): **столы-домики** — номер стола из списка `slicer_settings.house_tables` (миграция 030, по умолчанию 30/31/32, правится в Админке → Общие Настройки) рисуется на карточке кирпичным цветом в рамке в форме домика. Метится ТОЛЬКО номер стола; рамка карточки, мини-ряд разморозки, панель парковки и история не меняются. Попутно починен `PUT /api/settings`: после правки килсвитча синхронизации (ревью 2026-08-14) в запросе осталась дыра в нумерации параметров, и сохранение ЛЮБОЙ настройки падало 500-кой (см. Notes → «Нумерация параметров»).
>
> Предыдущая веха — 2026-08-15: **режим работы на 2–3 нарезчиков** — клейм карточки «В работе» переехал из локального стейта в БД (миграция 029) и виден на всех планшетах; своя карточка лаймовая, чужая голубая с именем; «Готово»/«Частично» на чужой — через подтверждение; в истории заказов и в отчёте «Скорость нарезчика» появился автор порции + фильтр по нарезчикам. Предыдущая веха — 2026-08-14: **полное ревью (7 зон) и правка 55 находок** — идемпотентность «Готово», окно ретенции истории, залипание экрана входа, ErrorBoundary, плашка «нет связи», таймзона и таймауты пула, каскад стоп-листа, безопасность (rate-limit входа, CORS, bind, загрузка файлов), миграции 027/028. Предыдущая веха — 2026-08-11, поиск по справочнику ингредиентов (паттерн 3); до неё 2026-07-12, аудит готовности к передаче. При изменении кода — обновляй этот файл (правило 2).
...
│   │   ├── SpeedKpiSection.tsx       # KPI скорости приготовления нарезчика + фильтр по нарезчикам и автор порции (029)
...
│       └── HouseTableBadge.tsx # Номер стола-домика: кирпичная цифра в SVG-контуре домика (миграция 030); общий для OrderCard и админки
...
│       ├── 029_order_claims.sql          # режим 2–3 нарезчиков: общий клейм «В работе» + автор порции в истории
│       └── 030_house_tables.sql          # столы-домики: house_tables в настройках (кирпичная метка номера стола)
...
14. **Режим 2–3 нарезчиков: общий клейм «В работе» (миграция 029)**
    - Тап по карточке помечает её «эту режу я». До 029 метка жила в локальном стейте `SlicerStation` и была видна только одному браузеру — при работе с разных планшетов не значила ничего. Теперь клейм хранится в `slicer_order_state` (`claimed_by_uuid` / `claimed_by_name` / `claimed_at`) и раздаётся всем планшетам обычным `GET /api/orders` (задержка до 4 сек; свой тап отображается мгновенно, оптимистично).
    - **Личность — из PIN-сессии** (`useAuth` → `App` → `useOrders`/`SlicerStation`). Требование к развёртыванию: у нарезчиков должны быть РАЗНЫЕ PIN в `users`, иначе клеймы неразличимы.
    - **Правила тапа:** свободная → беру; своя → отпускаю; чужая → тап игнорируется. Снять чужой клейм нельзя — по решению владельца он висит до «Готово» (парковка тоже снимает). Лимита на число взятых карточек нет.
    - **Цвета:** своя — лаймовая рамка (как было), чужая — голубая. У обеих подпись «🔪 имя» рядом с количеством порций.
    - **Мягкая защита:** «Готово» и «Частично» на чужой карточке открывают подтверждение «Карточку взял Азамат» → «Всё равно отдать». Парковка, отмена, разморозка вопросов не задают.
    - **Склеенная карточка атомарна.** В агрегированных режимах карточка = несколько позиций: клеймятся ВСЕ (`resolveSourceOrderIds`). Если хоть одна занята другим — весь клейм отклоняется (409 + имя владельца), потому что на соседнем планшете та же рыба может быть разбита иначе и «половина Азамата, половина Ержана» не читается. Занятой карточка считается, если занята хотя бы одна позиция; владелец — с самым ранним `claimed_at` (одинаково на всех устройствах).
    - **Гонка двух планшетов решается в БД**, а не в UI: `ON CONFLICT DO UPDATE ... WHERE claimed_by_uuid IS NULL OR = наш` под READ COMMITTED + финальная проверка на чужие клеймы внутри той же транзакции. Проигравший получает 409, откатывает оптимистичную метку и видит короткое уведомление.
    - **Оптимистичный оверлей** `pendingClaimRef` в `useOrders` — как у `merge_ack`: поллинг, стартовавший до коммита нашего POST, не откатывает метку.
    - **Жизненный цикл:** парковка снимает клейм, разморозка сохраняет (мини-карточка в `DefrostRow` тоже подписана), `/restore` сбрасывает.
    - **Автор порции в отчётах:** `slicer_order_history.completed_by_uuid/name` — тот, кто НАЖАЛ «Готово»/«Частично», а не тот, кто держал клейм (решение владельца: закрыть чужую карточку можно, в KPI должен попасть фактический исполнитель). В `SpeedKpiSection` имя видно в детализации порций и есть фильтр «нарезчик»; в Excel — колонка «Нарезчик» + пометка о применённом фильтре. Записи до 029 — «—».

15. **Столы-домики: кирпичная метка номера стола (миграция 030)**
    - Часть посадки ресторана — отдельные домики на улице (по умолчанию столы 30/31/32). Блюда туда уносят не в зал, нарезчику это важно видеть ещё при нарезке, а на доске все номера выглядели одинаково жёлтыми.
    - Список номеров — `slicer_settings.house_tables` (integer[], default `{30,31,32}`), правится в UI: **Админка → Общие Настройки → «Столы-домики»** (чипсы с × + поле «Добавить»). Отдельной таблицы нет намеренно: это список чисел без атрибутов, который читается тем же `GET /api/settings`, что фронт и так грузит при старте.
    - **Область действия — только номер стола.** Рамка карточки (ULTRA / VIP / клейм «В работе»), мини-ряд разморозки `DefrostRow`, панель парковки и история заказов НЕ меняются (решение владельца). Проп `houseTables` прокидывается из `SlicerStation` в `OrderCard` и в `DefrostModal` (та же карточка — номер должен выглядеть одинаково).
    - Рисует `components/ui/HouseTableBadge.tsx` — общий компонент доски и админки, чтобы владелец в настройках видел ровно то, что увидит нарезчик. Цифра кирпичного цвета `#E2725B` внутри SVG-контура домика; `preserveAspectRatio="none"` растягивает домик под ширину номера, `vector-effect="non-scaling-stroke"` держит толщину линии. Настоящий кирпич `#B22222` не взят намеренно: на тёмной карточке (#151d2c) он даёт контраст ~2.9:1 и не читается с трёх метров.
    - **Не путать с фиолетовой рамкой** вокруг номера — это другая метка, «стол парковался в течение смены». Домик её не отменяет: форма и обводка кирпичные, а заливка и цвет цифры внутри остаются фиолетовыми.
    - Валидация в три слоя: CHECK (≤100 элементов, без NULL), API (целые 1..9999, ≤100, дедуп + сортировка), UI (те же границы + сообщение под полем).
...
- **Громкость сигналов**: константы `DEFROST_PEAK_GAIN = 0.75` и `NEW_ORDER_PEAK_GAIN = 0.66` в `utils.ts` (2026-08-15 подняты втрое с 0.25/0.22 — на работающей кухне прежние не слышали; ×3 ≈ +9.5 дБ, на слух «вдвое громче»). Потолок — 1.0, дальше клиппинг вместо громкости; если мало, следующий шаг не gain, а тембр (square вместо sine) или повтор паттерна
...
- **Столы-домики**: `slicer_settings.house_tables` (миграция 030, default `{30,31,32}`) — номера столов, которые на карточке рисуются кирпичным в рамке-домике. Правится в UI (Админка → Общие Настройки), см. паттерн 15
...
# Миграции БД (все 30 по порядку, с ON_ERROR_STOP и --single-transaction:
#  падение любого файла откатывается целиком, частично применённого состояния не бывает)
# ⚠️ При добавлении миграции — ДОПИСАТЬ её в скрипт `migrate` в server/package.json.
#    Проверено 2026-08-15: 030 туда не попала, и `npm run migrate` останавливался
#    на 029. Модуль после такого прогона запускается и выглядит рабочим, но
#    PUT /api/settings падает 500-кой (нет колонки house_tables) — в админке
#    молча не сохраняется ни одна настройка. Симптом далёк от причины, поэтому
#    после правки скрипта сверяйте его хвост с последним файлом в migrations/.
...
- **Пользователь:** задаётся в `server/.env` (`DB_USER` / `DB_PASSWORD`), см. `server/.env.example`. Пароль стенда здесь намеренно НЕ указан: этот файл входит в архив, который уходит IT-команде заказчика (`Инструкция.md`, шаг 2.0), а раньше он предлагал им стендовое значение как рабочее. То же правило уже применено в `BD_docs/README.md`.
...
| `slicer_order_state` | Теневое состояние позиций: статус, стеки, парковка, effective_created_at, finished_at, defrost_*, merge_ack (022), claimed_by_* (029 — клейм «В работе») |
| `slicer_order_history` | Завершённые заказы (KPI, snapshot) + автор порции `completed_by_*` (029) |
...
`aggregation_window_minutes` (легаси, не используется), `history_retention_minutes`, `active_priority_rules` (jsonb), `course_window_seconds` (стандартный режим), `course_pace_seconds` (умная v2, миграции 023/024/025 — «окно уступки», default 600, CHECK 10..3600), `restaurant_open_time`, `restaurant_close_time`, `excluded_dates` (jsonb), `enable_aggregation` (режим скорости, паттерн 9), `enable_smart_aggregation`, `enable_kds_stoplist_sync`, `enable_defrost_sound`, `enable_new_order_sound` (звук нового заказа, миграция 026, default TRUE), `house_tables` (столы-домики, миграция 030, integer[], default `{30,31,32}`, CHECK ≤100 без NULL), `dessert_category_id`, `dessert_auto_park_enabled`, `dessert_auto_park_minutes`, `dessert_trigger_modifier_patterns` (text[]).

⚠️ **Нумерация параметров в `PUT /api/settings` должна быть сплошной** (найдено 2026-08-15). Когда килсвитч `enable_kds_stoplist_sync` убрали из UPDATE (ревью 2026-08-14), его `$10` оставили в массиве значений «чтобы не перенумеровывать остальные» — но в тексте запроса он исчез. Postgres выводит типы параметров из текста запроса и отвечал «не удалось определить тип данных параметра $10»: сохранение **любой** настройки падало 500-кой, а админка молча откатывала правку и перечитывала старые значения. Нумерация выправлена ($1..$18); при добавлении новых полей дыр не оставлять.
...
- `POST /api/orders/claim` — взять карточку в работу (миграция 029). Body `{orderItemIds, actorUuid, actorName}` — все реальные позиции карточки. **409 + имя владельца**, если хоть одна занята другим нарезчиком (карточка атомарна)
- `POST /api/orders/unclaim` — отпустить карточку. Body `{orderItemIds, actorUuid}`; снимает ТОЛЬКО свой клейм, чужой не трогает (отвечает `released: 0`)
...
- `GET/PUT /api/settings` — настройки. С миграции 030 в теле есть `houseTables` (столы-домики): массив целых 1..9999, не более 100 — иначе 400; сервер дедуплицирует и сортирует список. `enableKdsStoplistSync` игнорируется (килсвитч, см. выше)
...
- `GET /api/history/orders?from&to` **или** `?retentionMinutes=N` — история завершённых заказов. Без параметров отдаётся окно 120 минут, всегда с `LIMIT 2000`. Доска запрашивает окно «Хранение истории» раз в 30 сек, отчёты — конкретный период (ревью 2026-08-14: раньше запрос без параметров тянул ВСЮ таблицу каждые 4 секунды). В записях есть `completedByUuid`/`completedByName` — автор порции (миграция 029)
...
## Упаковка архива для передачи (чек-лист, 2026-08-15)

Инструкции заказчика предполагают ZIP «папки проекта». Если заархивировать
рабочую папку как есть, наружу уедут файлы, которых там быть не должно —
`.gitignore` защищает только git, а не архиватор.

**Убрать из архива обязательно:**

| Файл | Что внутри |
|---|---|
| `server/.env` | Боевой пароль к БД (в архив идёт только `.env.example`) |
| `.mcp.json` | Строка подключения к БД с паролем + токены MCP-серверов |
| `.env.local`, корневой `.env.example` | Легаси от скаффолда. Секретов нет, но корневой `.env.example` предлагает завести `.env.local` со своей строкой подключения — при развёртывании это сбивает с толку, единственный конфиг модуля `server/.env` |
| `.agent/` | Пустой служебный каталог инструментов |
| `.claude/`, `.serena/`, `.git/` | Внутренняя история, память AI-инструментов и локальные настройки разработки |
| `node_modules/`, `dist/`, `server/dist/` | Ставятся `npm install` на месте; сотни МБ |
| `code_review_08.04.2026.md`, `implementation_plan1.md` | Внутренние рабочие документы (в первом — легаси `PIN_CODE` из мока) |

**Оставить обязательно:** `server/migrations/` (все 30), `server/.env.example`,
`server/scripts/`, `BD_docs/`, `Инструкция.md`,
`Полная_инструкция_по_установке.txt`, `README.md`, оба `package.json` +
`package-lock.json`, `server/public/images/` с `.gitkeep`.

**Отдельно рядом с архивом:** `slicer_data_dump.sql` и папка с фото.
Комплект описан для заказчика в `Инструкция.md` → шаг 2.0 — там же список
того, чего в архиве быть не должно, чтобы они могли нас поправить.
...
6. ~~**Дамп с включённым килсвитчем и без `house_tables`**~~ — **закрыто 2026-08-15**: дамп пересобран с боевой БД. Что важно помнить при следующей пересборке (процедура записана в `Инструкция.md` → шаг 4.6, свёрнутый блок «Как этот дамп создаётся»): после `pg_dump` файл ОБЯЗАН пройти две правки, иначе возвращаются старые грабли — (1) `enable_kds_stoplist_sync` в строке `slicer_settings` выставить в `f` (на стенде флаг включён, а `slicer_kds_sync_config` в дамп не входит → первое нажатие на стоп роняет `loadSyncConfig`); (2) вычистить из шапки `\restrict` / `\unrestrict` / `SET transaction_timeout` (PG 17+, ломают восстановление на 13–16). Проверять пересозданный дамп **восстановлением в чистую БД**, а не глазами. Текущий состав: 6 категорий, 260 ингредиентов, 472 рецепта, 116 алиасов, 299 привязок категорий, 184 приоритета, 177 записей разморозки, 2 фото блюд, 1 строка настроек.
7. ~~**Дамп не восстановится на PostgreSQL ниже 17**~~ — **закрыто 2026-08-15** там же: несовместимые строки убраны из шапки, восстановление проверено прогоном в чистую БД (все 9 таблиц сошлись с источником, orphan-проверки по рецептам/иерархии сырья/категориям — нули).

   ⚠️ **Решение владельца по десертам (2026-08-15):** авто-парковка десертов уезжает в дамп **включённой с 15 минутами** — это боевая настройка ресторана, а не остаток тестов. Дефолт чистой установки (выключено / 40 минут) при этом не менялся, поэтому документация теперь явно разводит два случая: `Инструкция.md` шаг 4.6 (таблица «что приезжает с дампом») и шаг 9.6. Не «чинить» это обратно при следующей пересборке дампа.
