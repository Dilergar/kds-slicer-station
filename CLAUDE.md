...
> Актуализирован 2026-08-14: **полное ревью (7 зон) и правка 55 находок** — идемпотентность «Готово», окно ретенции истории, залипание экрана входа, ErrorBoundary, плашка «нет связи», таймзона и таймауты пула, каскад стоп-листа, безопасность (rate-limit входа, CORS, bind, загрузка файлов), миграции 027/028. Предыдущая веха — 2026-08-11, поиск по справочнику ингредиентов (паттерн 3); до неё 2026-07-12, аудит готовности к передаче. При изменении кода — обновляй этот файл (правило 2).
...
├── utils.ts                # Утилиты: generateId(), calculateConsumedIngredients(), withParentPrefix()/withoutParentPrefix() (приставка сырья, паттерн 3), playDefrostBeep(), playNewOrderBeep()
...
├── eslint.config.js        # ESLint flat config — сам eslint не установлен; скрипт npm run lint УБРАН из package.json (падал у заказчика)
...
│   │   ├── RecipeEditor.tsx     # Редактор рецептов (блюдо→ингредиенты, алиасы, приоритет, разморозка) + поиск по справочнику ингредиентов в модалке
...
│   │   ├── config/
│   │   │   └── db.ts       # pg.Pool подключение к arclient + обязательный pool.on('error') (см. ниже)
│   │   ├── routes/
...
│       ├── 027_history_fractional_quantity.sql   # total_quantity INT → NUMERIC(21,3): весовые позиции чека
│       └── 028_archive_trigger_never_blocks.sql  # триггер-архиватор в EXCEPTION-блоке + SECURITY DEFINER
...
3. **Иерархия ингредиентов + каскадный стоп блюд**
   ...
   - **Соглашение об именовании разновидностей (2026-07-29): `«<Сырьё> · <нарезка>»`** — например `Вешенки · Крупная соломка`. Приставка хранится в самом `slicer_ingredients.name`, а НЕ собирается при отрисовке. Причина: имя ингредиента выводится в семи местах, включая SQL на бэкенде (причина стопа `Missing: X` в `stoplist.ts`, выдача рецептов в `recipes.ts`) и снимок расхода в `slicer_ingredient_consumption` — денормализация закрывает все сразу и не даёт пропустить точку вывода. Без приставки на карточке заказа видно только «Кубик», а кубиком режут морковь, огурцы, лук, перец, говядину и судака.
     - Хелперы — `withParentPrefix()` / `withoutParentPrefix()` + `VARIETY_SEPARATOR` в `utils.ts` (обе идемпотентны).
     - Приставка поддерживается автоматически в `StopListManager`: проставляется при добавлении разновидности, снимается в поле редактирования и возвращается при сохранении, **переписывается во всех children при переименовании сырья** (`handleSaveParentName`) — иначе протухнет.
     - Внутри модалки сырья приставка прячется (`withoutParentPrefix`): название сырья уже в заголовке. Везде за её пределами имя показывается целиком.
     - **Поиск в модалке «Редактировать Рецепт»** (`RecipeEditor`, панель «Все Ингредиенты», 2026-08-11): справочник вырос до ~200 строк, глазами искать нереально. Фильтр пословный (`matchesSearchTokens`), а не по цельной подстроке — иначе «морковь кубик» не совпало бы с `Морковь · Кубик` из-за разделителя; порядок слов не важен. «ё» приравнена к «е» (`normalizeSearch`). Совпало сырьё → показываем его и ВСЕ нарезки (нужно выбрать подвид); совпали только нарезки → сырьё остаётся заголовком, под ним только подходящие. Нарезка ищется по склейке `«сырьё + имя»` — страховка на случай записей без приставки. Запрос сбрасывается при каждом открытии/закрытии модалки, иначе список откроется отфильтрованным и покажется что ингредиенты пропали.
...
10. **Авторизация по PIN из чужой таблицы `users`**
    ...
    - **Ограничение попыток входа (ревью 2026-08-14):** 10 неудач в минуту с одного адреса → 429, плюс задержка 400 мс на каждый неверный PIN. PIN — 4 цифры, то есть 9000 вариантов: без лимита любой в Wi-Fi ресторана вынимал боевые PIN сотрудников с логинами и ролями. Счётчик в памяти процесса (`server/src/routes/auth.ts`).
    - **Перепроверка сессии:** `GET /api/auth/me?uuid=…` дёргается при загрузке и раз в 10 минут (`useAuth`). Пользователь заблокирован/удалён → разлогин; сменились роли → обновляются на лету. Сетевая ошибка НЕ разлогинивает (обрыв Wi-Fi не повод выкидывать нарезчика посреди смены).
    - ⚠️ **PIN сотрудников в документации не хранится.** Раньше здесь была таблица с боевым PIN из таблицы `users` заказчика — удалена перед передачей. Нужный код смотрите прямо в БД: `SELECT login, pin FROM users WHERE locked = false AND pin > 0;`
...
# npm run lint — скрипт удалён (ревью 2026-08-14): eslint не в devDependencies,
#                 команда из README падала у заказчика. Конфиг eslint.config.js оставлен.
...
# Миграции БД (все 28 по порядку, с ON_ERROR_STOP и --single-transaction:
#  падение любого файла откатывается целиком, частично применённого состояния не бывает)
...
⚠️ **`enable_kds_stoplist_sync` НЕ меняется через `PUT /api/settings`** (ревью 2026-08-14). Это единственный предохранитель между модулем и записью в чужую `rgst3_dishstoplist`, а раньше он переключался тем же открытым запросом, что и громкость звука. Присланное значение игнорируется с предупреждением в лог. Менять только SQL-ом при развёртывании:
```sql
...
## API Endpoints
- `POST /api/auth/login` — авторизация по PIN. Body: `{pin: number}`. Возвращает `{uuid, login, roles}`, 401 при неверном PIN, **429 при 10 неудачах в минуту с одного адреса** (ревью 2026-08-14).
- `GET /api/auth/me?uuid=…` — перепроверка сессии (ревью 2026-08-14). 200 с актуальными `{uuid, login, roles}` либо 401, если пользователь заблокирован/удалён. Дёргается `useAuth` при загрузке и раз в 10 минут.
...
- `GET/POST/PUT/DELETE /api/ingredients` — CRUD ингредиентов. DELETE заодно удаляет фото своё и всех разновидностей
- `PUT /api/ingredients/:id/rename-parent` — транзакционное переименование сырья + перезапись приставок у всех разновидностей одним запросом (ревью 2026-08-14; раньше клиент слал N независимых PUT и обрыв связи оставлял половину со старой приставкой)
- `GET /api/ingredients/:id/usage` — `{children, recipes, activeStops}` для диалога подтверждения удаления: удаление сырья каскадит на разновидности, а те — на `slicer_recipes`
...
- `GET /api/history/orders?from&to` **или** `?retentionMinutes=N` — история завершённых заказов. Без параметров отдаётся окно 120 минут, всегда с `LIMIT 2000`. Доска запрашивает окно «Хранение истории» раз в 30 сек, отчёты — конкретный период (ревью 2026-08-14: раньше запрос без параметров тянул ВСЮ таблицу каждые 4 секунды)
- `DELETE /api/history/orders/:id` — удалить запись (для restore)
...
00. **Новые предохранители и файлы (ревью 2026-08-14):**
   - `components/ui/ErrorBoundary.tsx` — оборачивает `App` в `index.tsx`. Необработанное исключение рендера больше не даёт пустой белый экран: нарезчик видит крупную кнопку «Перезагрузить».
   - `components/ui/ClampedNumberInput.tsx` — числовое поле с ограничением при уходе из поля, а не на каждое нажатие. Используется для шага курса, окна COURSE_FIFO, хранения истории и минут разморозки. Прежний кламп в `onChange` не давал набрать 60 при минимуме 10 (первая же «6» превращалась в «10»).
   - `server/src/utils/imageUpload.ts` — общие правила приёма фото: расширение из whitelist по проверенному типу (НЕ из `originalname`) + сверка сигнатуры файла. Раньше `x.html` с заголовком `image/png` сохранялся как `.html` в папку статики.
   - `services/client.ts` — единый предел ожидания запроса (`DEFAULT_TIMEOUT_MS = 15000`). Без него зависший запрос держал флаг опроса и занимал слоты соединений браузера.
   - `server/src/index.ts` — `SERVER_HOST` (по умолчанию `127.0.0.1`), `CORS_ORIGINS`, заголовки `nosniff`/CSP на статике.
   - `server/src/config/db.ts` — `DB_TIMEZONE`, `DB_STATEMENT_TIMEOUT_MS`, `DB_IDLE_TX_TIMEOUT_MS`; пароль БД обязателен (фолбэк `'1234'` убран, процесс падает с внятным сообщением).

0. **`pool.on('error')` в `server/src/config/db.ts` — не удалять (добавлено 2026-08-11).** node-postgres эмитит `'error'` на пуле, когда рвётся ПРОСТАИВАЮЩЕЕ соединение: рестарт PostgreSQL, обрыв сети, `pg_terminate_backend`, отсечение простоя фаерволом. Пул — EventEmitter, необработанное `'error'` в Node не логируется, а выбрасывается — падает весь API-процесс. Со слушателем пул выбрасывает битого клиента и открывает новое соединение на следующем запросе. Проверено на живом сервере: `pg_terminate_backend` двух idle-соединений пула → процесс жив (тот же PID), `/api/orders` отвечает 200. На проде backend должен работать под супервизором (`pm2` / Windows Service / `systemd`) — см. `Инструкция.md` раздел 7.2.
...
4. **ESLint не установлен** — `eslint.config.js` есть, но скрипта `npm run lint` в package.json больше нет (удалён 2026-08-14, чтобы команда из документации не падала у заказчика)
5. **Изображения** — фото блюд/ингредиентов хранятся файлами на диске сервера (multer), в БД — пути. Unsplash-ссылки остались только в мёртвых моках `constants.ts`.
   - **Имя файла — `<id>_<Date.now()>.<ext>`, метка времени обязательна (2026-07-29).** Статика отдаётся с `Cache-Control: public, max-age=604800` (`index.ts`), поэтому при постоянном имени `<id>.<ext>` перезалитое фото не доезжало до планшета: новый файл лежал на диске, а браузер неделю показывал старый по неизменившемуся URL. Меняющееся имя = новый URL = кэш промахивается. Прежний файл удаляется существующей веткой «путь в БД ≠ новый путь» в обоих upload-хэндлерах (`routes/ingredients.ts`, `routes/dishes.ts`). Длинный кэш при этом сохранён намеренно — на кухонном планшете картинки должны открываться мгновенно.
...
10. **Polling заказов** — каждые 4 секунды; авто-разпарковка и авто-парковка десертов происходят на backend при каждом `GET /api/orders`. **История заказов — отдельным интервалом раз в 30 сек** и с окном ретенции (ревью 2026-08-14). **Справочники (блюда, категории, ингредиенты) обновляются раз в минуту** — раньше грузились один раз при открытии страницы, из-за чего новое блюдо не появлялось на доске сутками.
...
>
> **Полное ревью 2026-08-14** (7 зон: очередь, заказы, стоп-лист, справочники,
> отчёты, миграции, безопасность). Найдено 51, владелец выбрал 55 правок
> (включая мелочи), очередь по его требованию НЕ трогали. Исправлено:
> залипание экрана входа после первой ошибки PIN (эффект зависел от флага,
> который сам же взводил), неидемпотентное «Готово» (сторож перехода
> состояния), окно ретенции + LIMIT для истории заказов и стоп-листа,
> ErrorBoundary + валидация периода в отчётах, плашка «нет связи» и таймаут
> запросов, опрос справочников раз в минуту, серверный расчёт остатка при
> частичной отдаче, фильтр по смене в `GET /api/orders`, парковка десертов с
> прошедшим «Готовить к…», касты `::text` на чужих колонках и агрегация всех
> модификаторов, таймзона и `statement_timeout` в пуле, каскад стоп-листа
> (обновление причины, advisory-lock, отказ снимать каскадный стоп, единое имя
> блюда, дедуп alias-групп), админка (кнопка «Создать Рецепт» убрана, черновики
> числовых полей, счётчики при удалении категории и сырья, патч настроек,
> транзакционное переименование сырья), Excel (процент простоя, пометка о
> фильтре), безопасность (rate-limit входа, CORS, bind на 127.0.0.1, whitelist
> расширений + проверка сигнатуры файлов, килсвитч синхронизации вынесен из
> открытого API, пароль БД только из окружения), миграции 027/028 и
> `--single-transaction`.
>
> Владелец сознательно НЕ выбрал: пересборку дампа с выключенным флагом
> синхронизации и совместимость дампа с PostgreSQL < 17 (пункты 6–7 ниже),
> транзакционное удаление ингредиента с пересчётом каскада, пересчёт каскада
> при правке рецепта/алиасов, мёртвый тумблер снятия KDS-стопа при выключенной
> синхронизации, отсечку выбросов в скорости повара, состояние ошибки в
> отчётах, лимит строк Excel, двойной префикс приставки сырья.
>
> Отложенные пункты:
...
6. **🔴 В `slicer_data_dump.sql` включён `enable_kds_stoplist_sync = t`** (ревью 2026-08-14, находка №4 — владелец отложил). Дамп снят со стенда, где синхронизацию включали для тестов. У заказчика после восстановления дампа стоп-лист упадёт при первом нажатии: `loadSyncConfig` бросает исключение, потому что таблицы `slicer_kds_sync_config` в дампе нет. Противоречит обещанию в `Инструкция.md:1090` («выключена из коробки»), а проверка на шаге 4.5 идёт ДО восстановления дампа и покажет `false`. **Перед передачей** — либо пересобрать дамп с выключенным флагом, либо дописать в шаг 4.6: `UPDATE slicer_settings SET enable_kds_stoplist_sync = false WHERE id = 1;`. Там же `dessert_auto_park_enabled = t` и 15 минут, тогда как инструкция обещает выключено и 40.
7. **🔴 Дамп не восстановится на PostgreSQL ниже 17** (находка №5 — владелец отложил). Снят `pg_dump 18.3`, в шапке `SET transaction_timeout` (появился в PG 17) и мета-команды `\restrict` / `\unrestrict`. `Инструкция.md:5` заявляет «PostgreSQL 13+». С `ON_ERROR_STOP=1` восстановление обрывается на 13-й строке — уже ПОСЛЕ `TRUNCATE`, то есть база остаётся пустой и модуль виснет на «Загрузка настроек…». Сами миграции на 13 работают — достаточно вычистить эти строки из шапки дампа.
8. **Удаление ингредиента не пересчитывает каскад** (находка №16 — не выбрана). `DELETE /api/ingredients/:id` — голый DELETE без транзакции; `slicer_dish_stoplist.cascade_ingredient_id` объявлен `ON DELETE CASCADE`, поэтому строки стопа исчезают молча, включая блюда, которым не хватает ДРУГОГО стопнутого ингредиента. Закрывающей записи в истории нет, время простоя теряется. Чинится следующим toggle любого ингредиента.
9. **Правка рецепта и связывание алиасов не пересчитывают каскад** (находка №17 — не выбрана). Единственная точка пересчёта — `POST /api/stoplist/toggle`. Добавленный в рецепт стопнутый ингредиент не ставит блюдо на стоп; убранный — не снимает.
10. **При выключенной синхронизации тумблер снятия KDS-стопа блюда мёртв** (находка №19 — не выбрана). `isCurrentlyStopped` учитывает `rgst3` независимо от флага, поэтому выбирается ветка снятия, а `pushDishUnstopAll` при выключенной синхронизации — no-op. API отвечает `{toggled:true}`, UI верит и откатывается на следующем reload.
11. **Скорость повара без отсечки выбросов** (находка №30 — не выбрана). Забытая на раздаче отметка даёт замер в 6 часов; по блюду с 20 честными замерами по 5 минут среднее прыгает до 22 минут. Медианы рядом нет.
12. **Упавший запрос в отчётах выглядит как «данных нет»** (находка №31 — не выбрана). `Dashboard` в `catch` ставит пустой массив; кнопка Excel остаётся активной и выгружает файл с нулями.
13. **Excel без лимита строк** (находка №32 — не выбрана). Листы скоростей пишут строку на каждую порцию: месяц ≈ 18 тыс. строк, год — за сотню тысяч. Книга целиком строится в памяти планшета.
14. **Двойной префикс приставки сырья** (находка №33 — не выбрана). `withoutParentPrefix` снимает приставку только при точном совпадении с текущим именем родителя; после рассогласования получается «Вёшенки · Вешенки · Крупная соломка». Транзакционное переименование (№34, сделано) резко снижает вероятность рассогласования, но не исключает записи, заведённые до соглашения.
