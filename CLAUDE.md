...
| `.env.local` | Легаси от скаффолда, но лежит рядом с реальными конфигами |
| `.claude/`, `.git/` | Внутренняя история и локальные настройки разработки |
...
6. **🟡 `slicer_data_dump.sql` удалён из рабочего дерева и устарел** (обновлено 2026-08-15 при аудите передачи). В git файл ещё лежит (`git show HEAD:slicer_data_dump.sql`, 215 КБ), в рабочей папке его нет — все три документа обещают дамп в комплекте. Содержимое версии из git снято 2026-08-13, то есть ДО миграции 030: в `COPY slicer_settings` нет колонки `house_tables` (не страшно — подставится DEFAULT `{30,31,32}`), зато включены `enable_kds_stoplist_sync = t` и `dessert_auto_park_enabled = t` с 15 минутами, хотя инструкция обещает выключено и 40. Килсвитч особенно опасен: при первом же нажатии на стоп `loadSyncConfig` бросит исключение — таблицы `slicer_kds_sync_config` в дампе нет. **Документационная часть закрыта** — в `Инструкция.md` (шаг 4.6) и в `Полная_инструкция_по_установке.txt` (шаг 7.6) добавлен обязательный `UPDATE ... SET enable_kds_stoplist_sync = false, dessert_auto_park_enabled = false, dessert_auto_park_minutes = 40`. **Осталось решение владельца:** пересобрать дамп с актуальной боевой БД (тогда оба флага снимутся в источнике и войдут данные, наработанные после 13 августа) или вернуть версию из git и положиться на UPDATE из инструкции.
7. **🟡 Дамп не восстановится на PostgreSQL ниже 17** (находка №5). Снят `pg_dump 18.3`, в шапке `SET transaction_timeout` (появился в PG 17) и мета-команды `\restrict` / `\unrestrict`. С `ON_ERROR_STOP=1` восстановление обрывается на первых строках — уже ПОСЛЕ `TRUNCATE`, то есть база остаётся пустой и модуль виснет на «Загрузка настроек…». Сами миграции на 13 работают. **Документационная часть закрыта** (2026-08-15): и `Инструкция.md` (раздел 1), и `Полная_инструкция_по_установке.txt` (шаг 7) теперь явно говорят, какие три строки вычистить из шапки на PG 13–16. Радикальнее — снимать дамп с `--no-` совместимым pg_dump версии заказчика, но их версия неизвестна.
