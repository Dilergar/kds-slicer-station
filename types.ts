...
export interface IngredientBase {
  ...
  // Кто поставил ТЕКУЩИЙ стоп (миграция 014). Нужны секции «Активные сейчас»
  // в отчёте: до этого актор был виден только у уже снятых стопов, то есть
  // ответ на вопрос «кто и зачем поставил» появлялся, когда он уже не нужен.
  stopped_by_uuid?: string;
...
export interface Dish {
  ...
  // Для стопов кассира резолвится из rgst3.inserter через users.
  stopped_by_uuid?: string;
  stopped_by_name?: string;
...
export interface Order {
  ...
  // Чек чужой KDS (docm2_orders.suuid), которому принадлежит позиция.
  // Используется для группировки «волн» у заказов без стола (доставка,
  // самовывоз): один чек = один гость, независимые чеки не сцепляются.
  ...
  // TRUE = нарезчик уже подтвердил объединение этой позиции (кнопка Merge),
  // она рисуется внутри общего блока стека. FALSE/undefined = отдельный блок,
  // карточка показывает «N + 1» и требует Merge. Живёт в slicer_order_state —
  // переживает F5 и синхронно между планшетами.
  merge_ack?: boolean;

  // === Клейм «В работе» — режим 2–3 нарезчиков (миграция 029) ===
  // Кто взял позицию в работу. null/undefined = свободна. Живёт в
  // slicer_order_state, поэтому метку видят ВСЕ планшеты (раньше она была
  // локальным стейтом одного браузера и второй нарезчик про неё не знал).
  //   claimed_by_uuid — users.uuid из PIN-сессии, ключ сравнения «моя/чужая»;
  //   claimed_by_name — логин на момент клейма (для подписи на карточке);
  //   claimed_at — когда взяли (тайбрейкер и будущая авто-очистка).
  ...
  // === «Замороженные» курсы умной очереди (2026-07-11) ===
  // Контекст визита гостя (стол / чек доставки) с backend'а:
  //   visit_completed_dish_ids — канонические dish_id позиций, УЖЕ отданных
  //     нарезчиком в этом визите (по открытым чекам гостя; чек закрыт →
  //     визит окончен, контекст не отдаётся);
  //   visit_started_at — самое раннее время пробития среди отданных позиций
  //     (минимум с активными buildSmartQueue берёт сам).
  // Нужны, чтобы номер курса НЕ пересчитывался после «Готово»: курс = ранг
  // категории среди всех категорий визита, поэтому отдача супа не «повышает»
  // горячее и не пересобирает очередь. Поля одинаковы у всех заказов одного
  // гостя. undefined (старый backend / тест-заказы) — расчёт деградирует к
  // прежнему поведению «по оставшимся позициям».
  visit_completed_dish_ids?: string[];
  visit_started_at?: number;
}
...
export interface OrderHistoryEntry {
  ...
  // Автор порции (миграция 029) — тот, кто нажал «Готово»/«Частично», а не
  // тот, кто держал клейм: закрыть чужую карточку можно (с подтверждением),
  // и в KPI должен стоять фактический исполнитель. null у записей, сделанных
  // до режима нескольких нарезчиков — в отчёте показываются как «—».
  completedByUuid?: string | null;
...
export interface SystemSettings {
  // Легаси (2026-07-06): лимит времени слияния убран решением владельца —
  // в режиме скорости слияние безлимитное. Колонка осталась в БД, кодом
  // не используется, UI-поле удалено.
  aggregationWindowMinutes: number;
  historyRetentionMinutes: number;      // Время хранения истории (макс: 120)
  activePriorityRules: SortRuleType[];  // Порядок правил сортировки ['ULTRA', 'COURSE_FIFO']
  // Окно COURSE_FIFO в секундах. Дефолта нет — истина всегда в UI/БД
  // (Админка → «Ранжирование и Приоритеты»). Используется в СТАНДАРТНОЙ
  // сортировке (оба режима агрегации выключены) как размер FIFO-bucket'а.
  courseWindowSeconds: number;
  // Шаг курса умной очереди v2 («Темп курсов», миграции 023/024), в секундах.
  // Смысл (уточнён 2026-07-11): ОКНО УСТУПКИ — на сколько курс N стола
  // уступает дорогу первым курсам гостей, пришедших позже. Виртуальное время
  // позиции = время первого заказа гостя + номер_курса × шаг; после
  // наступления этого времени позицию уже никто не обгонит. Нарезку НЕ
  // замедляет: при свободной очереди блюдо режется сразу — это только
  // порядок при конкуренции. По умолчанию 600 (10 минут). Правится в UI
  // рядом с тумблером «Волновая Агрегация (Умная)».
  coursePaceSeconds?: number;
  ...
  // «Окно Агрегации» = РЕЖИМ СКОРОСТИ (buildSpeedQueue, реализован 2026-07-06):
  // без порядка категорий, безлимитное слияние одинаковых блюд, строгий FIFO
  // по первому заказу. По умолчанию: false.
  ...
  //    Оба OFF = стандартная сортировка без объединения.
  ...
  // Столы-домики (миграция 030): номера столов, вынесенных из зала в отдельные
  // домики. На карточке такой номер рисуется кирпичным цветом в рамке в форме
  // домика — нарезчик видит «это на улицу» прямо по номеру стола. Пустой
  // массив = меток нет. Правится в UI (Админка → Общие Настройки).
  // ❗ Метка касается ТОЛЬКО номера стола: рамка карточки, мини-ряд разморозки,
  //    панель парковки и история остаются без изменений (решение владельца).
  houseTables?: number[];               // По умолчанию: [30, 31, 32] (DEFAULT в БД)
...
export interface FlatOrderItem {
  ...
  // Ключ «гостя» для расчёта волн. Для позиций со столом — `t<номер>` (все
  // заказы стола = один гость). Для позиций без стола (доставка, самовывоз,
  // tableNumber=0) — `o<id чека>`: раньше все такие позиции сцеплялись в один
  // виртуальный «стол 0», и суп одной доставки задерживал горячее другой.
  waveKey: string;
